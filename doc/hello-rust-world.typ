...
- エラーハンドリングは、エラーが生じた場所ではなく、まとめて一箇所で行う方が嬉しい。
...
Rustでは、`Result`を返す関数内でのみ使える`?`演算子があります。これは、`Result`が`Ok`なら`Ok`の値を、`Err`ならその`Err`を即時`return`するという演算子です。これにより、エラーが発生した場合は即座にエラーを返して関数を終了することができます。以下のように、大まかなロジックを汚すことなくエラーを纏めることができます。
```rust
